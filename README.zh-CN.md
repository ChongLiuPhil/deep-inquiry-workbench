...
| `CL-` | 不能由 AI 静默猜测、且会实质影响探究的高影响歧义；它记录当前操作性不确定性，不替代人类 D- 决定 |
...
对于长程探究，最新的 `workspace.md` 仍然是权威状态，聊天摘要、较早的文件摘录和模型记忆只是临时缓存。工作文档现在用少量字段保存“从哪里继续”的操作恢复状态——当前焦点、首要阻塞、立即下一步和未解决的高影响 `CL-` 澄清——使零上下文的新 Agent 可以恢复工作，而不需要把每个探究项目拆成多文件治理仓库。CL- 首先解决“含混的指令或状态究竟是什么意思”；意思已经清楚后若仍有方向、价值或风险选择，则另由 D- 处理。
